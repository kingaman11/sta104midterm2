...
```{r average and sd_month, echo=FALSE}
...
```{r average and sd_age_group, echo=FALSE}
...
```{r average and sd_sex, echo=FALSE}
...
```{r Box-plot_month, echo=FALSE}

library(ggplot2)
boxplot(Death ~ Month, data = Covid_B_sub, main = "Covid-19 Deaths by Month", horizontal = TRUE)
```
...
```{r Box-plot_age_group, echo=FALSE}

boxplot(Death ~ Age_Group, data = Covid_B_sub, main = "Covid-19 Deaths by Age Group", horizontal = TRUE)
```
...
```{r Box-plot_sex, echo=FALSE}

boxplot(Death ~ Sex, data = Covid_B_sub, main = "Covid-19 Deaths by Sex", horizontal = TRUE)
```
...
```{r Contingency Table, echo=FALSE, warning=FALSE}
...
Here we can see our $\chi^2$ value above is 17.91532.


Next, we will proceed with obtaining a p-value by using the Permutation test:
...
As we can see above, our permutation based p-value is 1.
...
Next, we find the Tukey-Inspired cutoff value:
